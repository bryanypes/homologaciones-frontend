...
export default function SolicitudesVicerrector() {
  const navigate = useNavigate();
  const { showError } = useFeedback();
  const [solicitudes, setSolicitudes] = useState([]);
  const [total, setTotal] = useState(0);
  const [pagina, setPagina] = useState(1);
  const [filtroEstado, setFiltroEstado] = useState('pendiente_rector');
    ...
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content" tabIndex={-1} className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-wrap items-end gap-3 mb-6">
          ...
          {(filtroEstado !== 'pendiente_rector' || fechaDesde || fechaHasta) && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { setFiltroEstado('pendiente_rector'); setFechaDesde(''); setFechaHasta(''); }}
            >
              ...
              <table className="min-w-full divide-y divide-ink-100 text-sm">
                ...
                <tbody className="divide-y divide-ink-100">
                  {solicitudes.length === 0 ? (
                    ...
                    <ClickableRow key={s.id} onClick={() => navigate(`/vicerrector/solicitudes/${s.id}`)}>
